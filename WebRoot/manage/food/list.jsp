...
<div id="TitleArea">
	...
	<div id="TitleArea_Title">
		<div id="TitleArea_Title_Content">
			<img border="0" width="13" height="13" src="<%=basePath %>css/images/title_arrow.gif"/> 菜品列表
		</div>
    </div>
...
<div id="MainArea">
    <table class="MainArea_Content" cellspacing="0" cellpadding="0">
        ...
        <tbody id="TableData" align="center">
         <c:forEach var="dishs" items="${ requestScope.dishs}">
			<tr class="TableDetail1">
				<td width="100px">${dishs.id}</td>
				<td width="200px">${dishs.typeName}</td>
				<td width="200px">${dishs.dishName}&nbsp;</td>
				<td width="100px">${dishs.price}</td>
                <td width="100px">${dishs.vprice}</td>
				<td>是</td>
				<td>
					<a href="<%=basePath %>food/MFoodUpdateServlet?id=${dishs.id}&fname=${dishs.dishName}&price=${dishs.price}&vprice=${dishs.vprice}&dishdesc=${dishs.dishdesc}" class="FunctionButton">修改</a>
					<a href="<%=basePath %>food/deleteFoodServlet?fname=${dishs.id}" onClick="return delConfirm();"class="FunctionButton">删除</a>				
				</td>
			</tr>
		</c:forEach>	
        </tbody>
	...
	<!-- 其他功能超链接 -->
    <div id="TableTail">
		<div class="FunctionButton"><a href="<%=basePath %>food/MFoodSaveServlet">新建</a></div>
    </div>
</div>
		...
		<div id="PaginationBar_PageSelectorArea">
			<table id="PaginationBar_TablePageNum" cellpadding="0" cellspacing="5" width="330px">
				<tr>
			       ...
			       <c:otherwise>
				       <td align="right"><a href="<%=basePath %>food/MFoodPageServlet?currPage=${pc.previousPage }" title="首页"><img src="<%=basePath %>css/images/pagination/first_page.png"/></a></td>
		           </c:otherwise>
		           </c:choose>
			       ...
			       <c:otherwise>
				      <td align="left"><a href="<%=basePath %>food/MFoodPageServlet?currPage=${pc.totalPages }" title="尾页"><img src="<%=basePath %>css/images/pagination/last_page.png"/></a></td>
			       </c:otherwise>
		           </c:choose>
...
<script  type="text/javascript">
		...
		function gotoPage(index){
			window.location.href="<%=basePath %>food/MFoodPageServlet?currPage="+index;
		}
		function ChangePageSize(index){
			 window.location.href="<%=basePath %>food/MFoodChangeServlet?pageSize="+index;
		}
	</script>
